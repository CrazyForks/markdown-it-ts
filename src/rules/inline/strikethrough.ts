...
export function strikethrough_postProcess(state: StateInline): void {
  const delimiters = state.delimiters

  postProcess(state, delimiters)

  // Also check tokens_meta if present (similar to emphasis)
  const tokens_meta = (state as any).tokens_meta
  if (tokens_meta) {
    for (let curr = 0; curr < tokens_meta.length; curr++) {
