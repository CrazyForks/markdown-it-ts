...
// Use the local TypeScript implementation for demos
...
function buildScrollMap() {
  ...
  textarea.val().split('\n').forEach((str) => {
    ...
    const h = Number.parseFloat(sourceLikeDiv.css('height'))
    const lh = Number.parseFloat(sourceLikeDiv.css('line-height'))
    acc += Math.round(h / lh)
  })
...
const syncResultScroll = _.debounce(() => {
  const textarea = $('.source')
  const lineHeight = Number.parseFloat(textarea.css('line-height'))
...
function loadPermalink() {
  ...
  // sanitize for sure
  if (!['html', 'src', 'debug'].includes(defaults._view)) {
    defaults._view = 'html'
  }
}
...
$(() => {
  // highlight snippet
  if (window.hljs) {
    $('pre.code-sample code').each((i, block) => {
  ...
  $('.source').on('touchstart mouseover', () => {
    $('.result-html').off('scroll')
    $('.source').on('scroll', syncResultScroll)
  })

  $('.result-html').on('touchstart mouseover', () => {
    $('.source').off('scroll')
    $('.result-html').on('scroll', syncSrcScroll)
  })

  $('.source-clear').on('click', (event) => {
    $('.source').val('')
    updateResult()
