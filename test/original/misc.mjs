import markdownit, { withRenderer } from '../../src'
...
describe('API', function () {
  it('constructor', function () {
    ...
    const md = withRenderer(markdownit('commonmark', { html: false }))
  ...
  it('force hardbreaks', function () {
    const md = withRenderer(markdownit({ breaks: true }))
  ...
  it('xhtmlOut enabled', function () {
    const md = withRenderer(markdownit({ xhtmlOut: true }))
  ...
  it('xhtmlOut disabled', function () {
    const md = withRenderer(markdownit({ xhtmlOut: false }))
...
describe('Misc', function () {
  ...
  it('Zero preset should disable everything', function () {
    const md = withRenderer(markdownit('zero'))
...
describe('Url normalization', function () {
  it('Should be overridable', function () {
    const md = withRenderer(markdownit({ linkify: true }))
...
describe('maxNesting', function () {
  it('Block parser should not nest above limit', function () {
    const md = withRenderer(markdownit({ maxNesting: 2 }))
    assert.strictEqual(
      md.render('>foo\n>>bar\n>>>baz'),
  ...
  it('Inline parser should not nest above limit', function () {
    const md = withRenderer(markdownit({ maxNesting: 1 }))
    assert.strictEqual(
      md.render('[`foo`]()'),
      '<p><a href="">`foo`</a></p>\n'
    )
