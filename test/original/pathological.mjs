...
async function test_pattern (str) {
  const worker = new Worker(new URL('./pathological_worker_thread.mjs', import.meta.url), { type: 'module' })
    ...
    const result = await Promise.race([
      new Promise((resolve, reject) => {
        const onMessage = (msg) => {
          if (msg && msg.ok) {
            ...
            reject(new Error(msg.err || 'Unknown worker error'))
          } else {
            resolve(msg)
          }
        }
      ...
      new Promise((_, reject) => {
        const t = setTimeout(() => {
          ...
          worker.terminate().finally(() => reject(new Error('Terminated (timeout exceeded)')))
        }, 3000)
        // prevent keeping event loop alive
        t.unref?.()
      })
    ])
